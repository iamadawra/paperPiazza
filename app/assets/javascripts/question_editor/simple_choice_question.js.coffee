...
class SimpleChoiceQuestion extends StandardQuestion
  ...
  addChoice: (params) ->
    ...
    choice_status = @addPreviewField
      ...
      renderer: (elem, field) =>
        @updateCustomData()
        for choice_id in [0...@cntChoices]
          field = @getPreviewField "choice_status_#{choice_id}"
          if not field
            continue
          @updateFieldData "input_choice_#{choice_id}"
          is_correct = @getFieldData "input_choice_#{choice_id}"
          if is_correct
