...
describe SelectAllQuestion do

  before { @question = build(:select_all_question) }
  subject { @question }
  ...
  it "should mark the correct answer as correct" do
    @question.is_correct?(@question.answers).should be_true
  end

  it "should mark an incorrect answer as incorrect" do
    @question.is_correct?(@question.answers - [@question.answers[0]]).should be_false
    @question.is_correct?(@question.answers + [5]).should be_false
  end
  ...
  describe "when processing form input" do
    it "should turn an array of strings into an array of integers" do
      @question.preprocess_answer(["4", "5"]).should eq([4, 5])
    end

    it "should turn gibberish into an array of 0's" do
      @question.preprocess_answer(["asdas", "asdsaa"]).should eq([0, 0])
    end

    it "should turn whitespace into an empty array" do
      @question.preprocess_answer("").should eq([])
      @question.preprocess_answer("  ").should eq([])
      @question.preprocess_answer("\t  ").should eq([])
    end

    it "should turn nil into an empty array" do
      @question.preprocess_answer(nil).should eq([])
    end
  end
